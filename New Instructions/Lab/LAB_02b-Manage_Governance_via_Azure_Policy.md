...
1. **按兩下省略號按鈕並選取下列值，以指定範圍**。 完成時，按兩下 [ **選取** ]。 
    ...
    >**注意**：範圍會決定原則指派生效的資源或資源群組。 您可以在管理群組、訂用帳戶或資源群組層級上指派原則。 您也可以選擇指定排除專案，例如個別訂用帳戶、資源群組或資源。
...
1. 藉由指定下列設定來設定指派的**基本**屬性 (讓其他設定保留預設值)：
    ...
    >**注意**：[指派名稱] 會自動填入您選取的原則名稱，但您可加以變更。 [ **描述** ] 是選擇性的。 
...
1. 按一下 [檢閱 + 建立]，然後按一下 [建立]。
    ...
    >**注意**：現在您會嘗試在資源群組中建立 Azure 儲存體 帳戶，以確認新的原則指派生效。 您將建立記憶體帳戶，而不需要新增必要的標籤。 
...
1. 選取 [ **新增** ]，然後設定指派的其餘 **[基本]** 屬性。
    ...
    | 設定 | 值 |
    | --- | --- |
    | 指派名稱 | `Inherit the Role tag and its Infra value from the resource group if missing` |
    | 描述 | `Inherit the Role tag and its Infra value from the resource group if missing` |
    | 強制執行原則 | 啟用 |
...
1. 在 [**標記**] 刀鋒視窗上，請注意****，具有 Infra** 值的角色**已自動指派給資源。
