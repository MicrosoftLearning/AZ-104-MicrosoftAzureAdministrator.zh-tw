...
在此工作中，您將透過 Azure 入口網站建立標籤，並將其指派給 Azure 資源群組。 卷標是治理策略的重要元件，如 Microsoft Well-Architected Framework 和 雲端採用架構 所述。 卷標可讓您快速識別資源擁有者、日落日期、群組聯繫人，以及貴組織認為重要的其他名稱/值組。 在此練習中，您將指派一個標記來識別資源角色 （'Infra' for 'Infrastructure'）。
...
1. 流覽回您在上一個工作中建立之資源群組的刀鋒視窗，然後選取 [ **卷標** ] 刀鋒視窗。
...
1. 布建新的記憶體帳戶之後，按兩下 [ **移至資源** ] 按鈕。 在 [概**觀]** 刀鋒視窗上，請注意，具有 Infra** 值的角色**標籤**** 已自動指派給資源。
