...
您的組織有公用網站。 您必須在不同虛擬機之間平衡傳入的公用要求。 您也需要提供來自不同虛擬機的影像和影片。 您計劃實作 和 Azure Load Balancer 和 Azure 應用程式閘道。 所有資源都位於相同的區域中。 
...
1. 使用下列資訊來完成 連線 ion 疑難解答**頁面上的**欄位。
    ...
    ![顯示 連線 疑難解答設定的 Azure 入口網站。](../media/az104-lab06-connection-troubleshoot.png)

1. 選取 [ **執行診斷測試**]。

    >**注意**：可能需要幾分鐘的時間，結果才會傳回。 收集結果時，螢幕選取專案會呈現灰色。 **請注意，連線 性測試**會顯示 **Reachable**。 這很合理，因為虛擬機位於相同的虛擬網路中。 
...
>**注意**：此 應用程式閘道 在與先前工作中的Load Balancer相同的虛擬網路中運作。 這在生產環境中並不常見。
...
1. 按一下 [儲存]
    ...
    > **注意**：此子網將由 Azure 應用程式閘道 實例使用。 應用程式閘道需要大小為 /27 或更大的專用子網路。 
...
1. 按一下 [Next: Backends >] \(下一步: 後端 >\)，然後按一下 [新增後端集區]。 這是映像**的後端集區**。 指定下列設定 (將其他設定保留為預設值)。 完成時，按一下 [新增]。
...
1. 切換至 [Backend targets] \(後端目標\) 索引標籤，並指定下列設定 (將其他設定保留為預設值)。 
   ...
   >**注意：** 使用參考圖示來深入瞭解 **Cookie 型親和**性和** 連線 清空**。 
...
1. 在 應用程式閘道 選取 [**後端健康情況 **]。****

1. 請確定後端集區中的兩部伺服器都顯示狀況 **良好**。 

1. 在 **az104-appgw** 應用程式閘道 刀鋒視窗中，複製 Frontend 公用 IP 位址**的值**。
