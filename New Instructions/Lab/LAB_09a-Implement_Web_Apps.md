...
在這個工作中，您將建立一個 Azure Web 應用程式。 Azure 提供 Azure App 服務，這是適用於 Web、行動和其他 Web 應用程式的平臺即服務 （PAAS） 解決方案。 Azure Web Apps 是一種 Azure App 服務 供應專案，可用於執行大部分運行時間環境的網站，例如 PHP、Java、.NET 等等。 如果您需要多個運行時間環境的支援，您可以使用App Services搭配 Docker 容器。 您選取的 SKU 會決定您透過 Web 應用程式收到的計算、記憶體和功能數量。
...
1. 在 [**設定]** 索引卷標的 **[來源**] 下拉式清單中，選取 **[外部 Git**]。
