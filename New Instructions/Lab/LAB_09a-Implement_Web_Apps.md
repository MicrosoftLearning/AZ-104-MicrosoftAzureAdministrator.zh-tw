...
貴組織對 Azure Web 應用程式感興趣，可裝載貴組織的網站。 網站目前裝載於公司內部部署數據中心。 該網站使用 PHP 執行階段堆疊在 Windows 伺服器上執行。 硬體即將結束生命週期，很快就會需要更換。 您的組織想要完成測試，以協助在生命周期結束日期之前移至 Azure。
...
在這個工作中，您將建立一個 Azure Web 應用程式。 Azure 提供 Azure App 服務，這是 Web、行動和其他 Web 應用程式的平台即服務 （PAAS） 解決方案。 Azure Web Apps 是一種 Azure App 服務 供應專案，可用於執行大部分運行時間環境的網站，例如 PHP、Java、.NET 等等。 如果您需要多個運行時間環境的支援，您可以使用App Services搭配 Docker 容器。 您選取的 SKU 會決定您透過 Web 應用程式收到的計算、記憶體和功能數量。
...
在這項工作中，您將設定 Web 應用程式部署設定。 您可以使用部署設定來設定應用程式服務，以允許從您選擇的存放庫持續部署，或使用 FTPS 認證和其他自動化。 這可確保App Service 具有執行中應用程式的最新版本。

1. 在預備部署位置刀鋒視窗的 **[設定**] 區段中，選取 **[組態**]，然後選取 [**一般設定**]。

    >**注意：** 請確定您位於預備位置刀鋒視窗上（而不是生產位置）。
    
1. 在 [**設定] 索引標籤**的 **[來源**] 下拉式清單中，選取 **[外部 Git**]。
