...
此實驗室是專注于虛擬網路的三個實驗室中的第一個。 在此實驗室中，您將瞭解虛擬網路和子網的基本概念。 您也會瞭解如何使用網路安全性群組和應用程式安全性群組保護您的網路。 
...
您的通用群組織計畫實作虛擬網路。 這些網路位於美國東部、西歐和東南亞。 立即的目標是要容納所有現有的資源。 不過，組織處於成長階段，並想要確保有額外的成長容量。

**CoreServicesVnet** 虛擬網路會部署在**美國東部**區域。 此虛擬網路的資源數目最多。 網路可透過 VPN 連線連線至內部部署網路。 此網路具有 Web 服務、資料庫和其他系統，這些系統是業務營運的關鍵。 共用服務，例如網域控制站和 DNS 位於這裡。 預期會有大量成長，因此此虛擬網路需要大型位址空間。

**ManufacturingVnet** 虛擬網路部署在**西歐**區域，靠近您組織的製造設施位置。 此虛擬網路包含製造設施作業的系統。 組織預期其系統需要大量的內部連線裝置來擷取資料，例如溫度，而且需要可擴充的 IP 位址空間。
...
+ [實作 ](https://mslabs.cloudguides.com/en-us/guides/AZ-104%20Exam%20Guide%20-%20Microsoft%20Azure%20Administrator%20Exercise%208) 虛擬網路。 建立和設定虛擬網路、部署虛擬機器、設定網路安全性群組，以及設定 Azure DNS。

## 架構圖

![網路設定](../media/az104-lab04-arch-diagram.png)

這些虛擬網路和子網的結構化方式可容納現有的資源，但允許預測的成長。 讓我們建立這些虛擬網路與子網路，為我們的網路基礎結構奠定基礎。

>**您知道嗎？ ** ：最好避免重迭的 IP 位址範圍，以減少問題並簡化疑難排解。 重迭是整個網路的問題，無論是雲端還是內部部署。 許多組織都會設計全企業 IP 定址配置，以避免重迭並規劃未來成長。
...
組織會規劃核心服務的大量成長。 在這項工作中，您會建立虛擬網路和相關聯的子網，以容納現有的資源和計畫成長。

1. 搜尋並選取 ** [虛擬網絡 ** ]。

1. 選取 ** [虛擬網路] 頁面上的 [建立 ** ]，然後完成 ** [基本] ** 和 ** [IPv4 位址 ** ]。 
...
1. 選取 ** [+ 新增子網 ** ]。 完成每個子網的名稱和位址資訊。 請務必為每個新子網選取 [ ** 新增 ** ]。 
...
1. 在 [自動化 ** ] 區 ** 段中，選取 ** [匯出範本 ** ]，然後等候產生範本。

1. **下載 ** 範本。

1. 在本機電腦上流覽至 ** [下載] ** 資料夾，然後 ** 解壓縮 ** 所下載 zip 檔案中的所有檔案。 

1. 繼續之前，請確定您有兩個 ** 檔案 template.json ** 和 ** parameters.json ** 。 花一分鐘的時間檢閱檔案和 CoreServicesVnet 的相關資訊。 您將使用此範本在下一個工作中建立 ManufacturingVnet。 
 
## 工作 3：建立 ManufacturingVnet 虛擬網路和子網路

在這項工作中，您會建立 ManufacturingVnet 虛擬網路和相關聯的子網。 組織預期製造業辦公室的成長，因此子網的大小會隨著預期的成長而調整規模。

1. 找出在 ** 上一個工作中匯出的 template.json ** 檔案。 它應該位於 [ ** 下載] ** 資料夾中。
...
在這項工作中，我們會建立應用程式安全性群組和網路安全性群組。 NSG 會有允許來自 ASG 流量的輸入安全性規則。 

### 建立應用程式安全性群組 （ASG）

1. 在Azure 入口網站中，搜尋並選取 ** [應用程式安全性群組 ** ]。
...
1. 按一下 [ ** 檢閱 + 建立 ** ]，然後在驗證之後按一下 [ ** 建立 ** ]。

### 建立網路安全性群組並將它與 ASG 子網產生關聯

1. 在Azure 入口網站中，搜尋並選取 ** [網路安全性群組 ** ]。
...
1. 建立 NSG 之後，按一下 ** [移至資源 ** ]。

1. 在 ** [設定下按一下 ** [ ** 子網 ** ]，然後按一下 [ ** 關聯 ** ]。
...
1. 在 [ ** 新增輸入連接埠規則] ** 刀鋒視窗上，使用下列資訊來新增輸入連接埠規則，然後選取 [ ** 新增 ** ]。
...
+ 虛擬網路是雲端中您自己的網路標記法。 
+ 設計虛擬網路時，最好避免重迭的 IP 位址範圍。 這可減少問題並簡化疑難排解。
+ 子網路是虛擬網路中的一個 IP 位址範圍。 您可以針對組織和安全性，將虛擬網路分成多個子網路。
+ 網路安全性群組包含允許或拒絕網路流量的安全性規則。 有預設的傳入和傳出規則，您可以自訂您的需求。
+ 應用程式安全性群組是用來保護具有一般功能的伺服器群組，例如網頁伺服器或資料庫伺服器。 
