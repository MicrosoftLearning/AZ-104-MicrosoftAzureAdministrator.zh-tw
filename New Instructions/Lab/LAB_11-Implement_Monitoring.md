...
1. 虛擬機器代理程式需要幾分鐘的時間才能安裝和設定，請繼續進行下一個步驟。 
   
## 工作 2：建立 Azure 活動記錄警示

在這項工作中，您會在刪除虛擬機器時建立警示。 
...
1. 選取 az104-rg11 資源群組的 ** 方塊，然後選取 [ ** 套用 ** ]。 ** 此警示會套用至資源群組中的任何虛擬機器。 或者，您可以只指定一部特定電腦。 

1. 選取 [條件] ** 索引 ** 標籤，然後選取 ** [查看所有訊號 ** ] 連結。

1. 搜尋並選取 [ ** 刪除虛擬機器] [虛擬機器]。 ** 請注意其他內建訊號。 選取 [套用]
...
1. 在 [ ** 建立警示規則 ** ] 窗格中，選取 [ ** 下一步：詳細資料 ** ] 按鈕，然後為每個設定輸入下列值。
...
>**注意： ** 如果您在部署警示規則之前刪除虛擬機器，則可能不會觸發警示規則。 
...
1. 在標題列中，選取 ** [通知] ** 圖示，並等候 ** vm0 ** 成功刪除。

1. 您應該會收到已讀取的通知電子郵件， ** 重要通知：已啟動 Azure 監視器警示 VM... ** 如果沒有，請開啟您的電子郵件程式，並尋找來自 azure-noreply@microsoft.com 的電子郵件。
...
1. 在 [ ** 查詢] ** 視窗中，選取 ** [虛擬機器 ** ] （左窗格）。 如有必要，請關閉啟動顯示畫面。 

1. 撤銷可用的查詢。 **執行 [將 ** 滑鼠停留在查詢上] ** 計數活動訊號 ** 查詢。

1. 您應該會在虛擬機器執行時收到活動訊號計數。

1. 檢閱查詢。 此查詢會 * 使用活動訊號 * 資料表。 

1. 將查詢取代為此查詢，然後按一下 [ ** 執行 ** ]。 檢閱產生的圖表。
