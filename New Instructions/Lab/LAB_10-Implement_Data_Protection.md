...
您的組織正在評估 Azure 復原服務，以備份和還原 Azure 虛擬機。 組織想要識別保護數據免於意外或惡意數據遺失的方法。
...
+ 工作 4：監視 Azure 備份
+ 工作 5：為虛擬機實作 Azure Site Recovery
...
在這項工作中，您將建立復原服務保存庫。 復原服務保存庫提供 Azure 虛擬機的備份數據。
...
1. 在 **[az104-vault1** Recovery Services 保存庫] 刀鋒視窗的 **[設定**] 區段中，按兩下 **[屬性**]。

1. 在 **az104-vault1 - [屬性]** 刀鋒視窗上，按兩下 **[備份組態 **] 標籤下的 **[更新]** 連結。

1. 在 [**備份組態**] 刀鋒視窗中，檢閱 儲存體 複寫類型的**選擇**。 保留 [異地備援] 的預設設定，並關閉刀鋒視窗。

    >**注意**：此設定只有在無現有備份項目時才能設定。

1. **回到 az104-vault1 - [屬性]** 刀鋒視窗，按兩下 **[安全性] 設定** 標籤下的 **[更新**] 連結。

1. 請注意，在 [安全性設定] 刀鋒視窗上，針對在 Azure 中執行的工作負載，[虛刪除] 為 [啟用]。

1. 關閉 [**安全性 設定]** 刀鋒視窗，然後回到 **az104-vault1** 復原服務保存庫刀鋒視窗，按兩下 **[概觀**]。
...
1. 在 **[az104-vault1** 復原服務保存庫] 刀鋒視窗上，按兩下 **[概觀**]，然後按兩下 [ **+ 備份**]。
...
1. 流覽回 **az104-vault1** 復原服務保存庫刀鋒視窗，在 **[受保護的專案** ] 區段中，按兩下 **[備份專案**]，然後按兩下 **[Azure 虛擬機** ] 專案。
...
1. 在 **az104-vault1** 頁面上，選取 [**診斷 設定**]。

1. 在 [診斷 設定] 頁面上，選取 [**新增診斷設定**]。

1. 在 [診斷設定] 頁面上，將設定 `Logs and Metrics to storage`命名為 。

1. 將複選標記放在下列記錄檔和計量分類旁邊：
...
1. 找出 az104-10-vm0** VM 的**備份作業，然後選取 [**詳細數據]**

    >**注意**：視螢幕解析度而定，您可能需要在備份作業數據表中向右卷動。

1. 檢閱您在 VM 上觸發的備份作業詳細數據。 備份作業的兩個子工作為何？
