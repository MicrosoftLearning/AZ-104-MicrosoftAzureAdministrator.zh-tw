...
在這項工作中，我們會在 Azure 入口網站 中建立受控磁碟。 受控磁碟是專為與虛擬機搭配使用的記憶體。 部署磁碟之後，您將匯出可在其他部署中使用的範本。
...
1. 在編輯器窗格中，進行這些變更。
    ...
    + 將az104_disk1**變更**為 `az102_disk2` （一個要變更的地方）

1. 請注意，這是 **標準** 磁碟。 位置為 **eastus**。 磁碟大小為 **32 GB**。

1. **儲存**您的變更。

1. Dpn 別忘了參數檔案。 選取 [ **編輯參數**]，按兩下 **[載入檔案** ]，然後上傳 **parameters.json**。 
...
1. 在 [ **您沒有掛接存儲設備]** 畫面上，選取 [ **顯示進階設定** ]，並提供必要資訊。 
    ...
    |  -- | -- |
    | 資源群組 | **az104-rg3** |
    | 儲存體 帳戶 （新建） | `sacloudshell` （必須是全域唯一的，長度介於 3 到 24 個字元之間，且只使用數位和小寫字母） |
...
1. 選取 [ **編輯器** ] （大括弧） 圖示，然後流覽至參數 JSON 檔案。
