...
## 作業 3：設定 Cloud Shell 並使用 PowerShell 部署範本 
...
1. 在 [**開始使用**] 畫面上，選取 [**掛接儲存體帳戶**]，選取您的**儲存體帳戶訂用帳戶**，然後選取 [**套用]。**

1. 選取 [我想要建立儲存體帳戶 **]，然後**選取 [** 下一步**]。 完成 [建立儲存體帳戶**資訊]。** 
...
1. 選取 [設定]** （頂端列），然後**選取 [** 移至傳統版本**]。

1. 選取 [上傳/下載檔案 **] **圖示 （頂端列），然後選取 **[上傳]。**

1. 從 Downloads** 目錄上傳**範本和參數檔案。 

1. 選取 **編輯器 （大括弧）** 圖示，然後導覽至導覽窗格左側的範本 JSON 檔案。
...
1. 選取 [管理檔案 **]，然後**選取 **[將 Bicep 檔案上傳**至 Cloud Shell]。 

1. 按一下**編輯器**，並在出現**** 提示時確認切換至傳統 Cloud Shell。

1. 選取 **azuredeploydisk.bicep** 檔案 

1. 請花一分鐘時間閱讀 Bicep 範本檔案。 請注意磁碟資源的定義方式。 
   
1. 進行下列變更：

    + 將 managedDiskName** 值第 2 行變更**為 **az104-disk5**。
    + 將 **SKU 名稱** 值第 26 行變更為 **StandardSSD_LRS**。
    + 將 diskSizeinGiB** 值;第 **7 行變更為 **32**。
