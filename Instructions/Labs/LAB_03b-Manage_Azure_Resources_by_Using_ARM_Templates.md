...
1. 在 **[開始使用]** 畫面上，選取 **[掛接儲存體帳戶]**，然後選取 **[我想建立儲存體帳戶]**。  

    >**注意：** 在此實驗室中，儲存體帳戶為必要項。 提供必需資訊。 
...
1. 完成後，請選取 **[下一步]**。 只有在第一次使用 Cloud Shell 時才需要執行此操作。 佈建儲存體需要幾分鐘的時間。

1. 使用 **[上傳/下載檔案]** 圖示，從下載目錄中上傳範本和參數檔案。 您需要單獨上傳每個檔案。

   >**注意：** 只要系統提示您 **[切換至傳統 Cloud Shell]**，即請依照提示進行。 
...
1. 確認您的檔案可在 Cloud Shell 儲存體中使用。 
    ...
    dir
    ```
    >**注意**：如果需要，您可以使用 **cls** 清除命令視窗。 您可以使用方向鍵來移動命令歷程記錄。
