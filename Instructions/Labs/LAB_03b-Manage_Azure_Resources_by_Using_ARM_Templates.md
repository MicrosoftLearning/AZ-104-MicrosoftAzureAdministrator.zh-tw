...
1. 使用 檔案總管 將下載檔案的內容解壓縮到**您電腦上的 [下載**] 資料夾中。 請注意，有兩個 JSON 檔案（範本和參數）。 
...
1. 在 [ **開始使用]** 畫面上，選取 [ **掛接記憶體帳戶** ]，然後 **我想建立記憶體帳戶**。  

    >**注意：** 針對此實驗室，需要記憶體帳戶。 提供必要資訊。 
...
1. 完成時，請選取 [ **下一步**]。 您只需要在第一次使用 Cloud Shell 時執行這項操作。 布建記憶體需要幾分鐘的時間。

1. **使用上傳/下載檔案**圖示，從下載目錄上傳範本和參數檔案。 您必須個別上傳每個檔案。

   >**注意：** 如果隨時提示您 **切換至傳統 Cloud Shell**，請執行此動作。 
