...
在此實驗室中，您將了解 Azure Web 應用程式。 您將了解如何設定 Web 應用程式，以在外部 GitHub 存放庫中顯示 Hello World 應用程式。 您將了解如何建立預備位置，並與生產位置交換。 您也會了解自動縮放以因應需求變更。
...
1. 回到 Web 應用程式的 [部署位置]**** 刀鋒視窗，按一下代表新建立預備位置的項目。
...
1. 在 Web 應用程式的 [概觀]**** 刀鋒視窗中，選取 [預設網域]**** 連結以顯示網站首頁。

1. 確認生產網頁會顯示 **Hello World！** 頁面。
...
1. 從 [概觀]**** | [Add HTTP requests]****，選取 [建立]****。

1. 針對 [測試 URL]****，貼上您的 **預設網域** URL。 請確定 URL 已正確格式化，並以 **https://** 開頭。

1. 選取 [檢閱 + 建立]****，然後選取 [建立]****。

    >**注意：** 建立測試可能需要幾分鐘的時間。 

1. 檢閱測試結果，包括 [虛擬使用者]****、[回應時間]****，以及 [要求/秒]****。
