...
1. 在 [建立 Web 應用程式] 刀鋒視窗的 [基本] 索引標籤上，指定下列設定 (其他設定請保留預設值)：
 ...
 1. 按一下 [檢閱 + 建立]****，然後按一下 [建立]****。
    ...
    >**注意**：如果部署失敗，請變更為另一個區域，然後再試一次。 這是因為不同區域中的配額。  
...
1. 檢閱預設設定，然後按兩下 [ **開始交換**]。 等候交換已完成的通知。

1. 返回入口網站首頁。 您應該同時擁有生產 Web 應用程式和預備位置。

1. 搜尋 `App Services` 並選取您的 App Service Web 應用程式。 這會將您傳回生產部署位置。

1. 選取 App Service Web 應用程式，然後在 Web 應用程式的 [概觀 **] 刀鋒視窗上**，選取 **[預設網域**] 連結以顯示網站首頁。

1. 確認生產網頁現在會顯示 **Hello World！** 頁面。
...
1. 在 [設定]**** 區段中，選取 [擴增 (App Service 方案)]****。
...
1. 從 [**概觀** | **建立]，新增 HTTP 要求**，選取 [**建立]。**

1. 在 [ **測試計劃]** 索引標籤上，按兩下 [ **新增要求**]。 在 [ **URL] 欄位中**，貼上您的 **預設網域** URL。 請確定 URL 已正確格式化，並以 **https://** 開頭。 選取 [新增]**** 以儲存您的變更。 
