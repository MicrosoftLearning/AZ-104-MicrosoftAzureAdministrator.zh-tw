...
1. 在 [部署中心] 刀鋒視窗上，選取 [本機 Git/FTPS 認證] 索引標籤，在 [使用者範圍] 區段中指定下列設定，然後按一下 [儲存]。
    ...
    | 設定 | 值 |
    | --- | ---|
    | 使用者名稱 | 任何全域唯一名稱（請參閱附註）  |
    | 密碼 | 任何符合複雜度需求的密碼（請參閱附註） |

    >**注意：** 將這些認證複製到 記事本。 您稍後會用到這些檔案。
    
    >**注意：** 這些認證會透過URI傳遞。 請勿包含任何會影響 URI 解譯的特殊字元。 例如，@、$或 #。 星號或加號 （在字串中間） 會運作。
...
1. 從 [Cloud Shell] 窗格中，執行下列命令以新增遠端 Git（請務必將 和 `[git_clone_uri]` **佔位符分別取代`[deployment_user_name]`為部署認證**使用者名稱和 Git Clone Uri** 的值**，而您在上一個工作中識別的值）：
...
1. 從 [Cloud Shell] 窗格中，執行下列命令，將範例 Web 應用程式程式代碼從本機存放庫推送至 Azure Web 應用程式預備部署位置（請務必將佔位符值取代為部署**認證使用者名稱和密碼的值**，以及您在上一個工作中識別的應用程式名稱）：
...
1. 在預備位置刀鋒視窗上，按兩下 **[概觀** ]，然後按兩下 [ **預設網域** ] 連結，在新的瀏覽器索引標籤顯示預設網頁。
...
1. **按兩下 **Web 應用程式生產位置刀鋒視窗上的 [概觀]，然後按兩下 **[預設網域**] 連結，在新的瀏覽器索引標籤中顯示網站首頁。
...
1. 從 [ **調整] 區段中** 選取 [ **以規則為基礎的** ] 選項，然後按兩下 [ **管理規則型調整** ] 連結。
...
1. 選取 [**自動調整 設定**]，選取 [**執行歷程記錄**] 索引標籤。在 [顯示最後一個數據] 底**下，選取 **[1 小時**]，然後檢查 **[觀察到的資源實例計數**]。**
