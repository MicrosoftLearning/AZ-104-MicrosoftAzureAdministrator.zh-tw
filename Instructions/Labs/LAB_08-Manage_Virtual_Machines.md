...
1. 將虛擬機器大小設為 **D2ds_v4** ，然後點選 **「調整大小**」。 出現提示時，請確認變更。

    >**注意**：若 **無法D2ds_v4** 尺寸，請選擇其他尺寸。 調整大小也稱為垂直擴大或縮小。
...
1. 在 **網路** 頁面，選擇 **編輯虛擬網路** 連結。 做些改變。 完成後，選取 [確定]****。

    | 設定 | 值 |
    | --- | --- |
    | 名稱 | `vmss-vnet` |
    | 位址範圍 | `10.82.0.0/20` （刪除現有的地址範圍） |
    | 子網路名稱 | `subnet0` |
    | 子網路範圍 | `10.82.0.0/24` |
...
1. 在 [建立負載平衡器] 頁面上，指定負載平衡器名稱並採用預設設定。 完成後點選**建立，然後**下一步：管理>****。
...
1. 讓我們建立一個規則，以自動增加 VM 執行個體的數目。 當平均 CPU 負載在 10 分鐘的期間內大於 70% 時，此規則會擴增。 當規則觸發時，虛擬機實例數量會增加 50%。
