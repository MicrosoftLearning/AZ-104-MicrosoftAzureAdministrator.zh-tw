...
1. 在 [**執行命令腳本]** 刀鋒視窗中，輸入下列命令，然後按兩下列命令，然後按兩下 [**執行**] 以存取 az104-08-vm1** 上**裝載的網站：
   ...
   ```
    ...
    >**注意**：-URI** 參數是 **VM 的私人 IP 位址****。 流覽至 **[網络 **] 區段中的 [az104-08-vm1]** 刀鋒**視窗，然後按兩下 [**網络設定]**
...
1. 使用下列設定建立受控磁碟（讓其他人保留預設值），然後按兩下列設定建立受控磁碟（讓其他人保留預設值），然後按兩下列設定建立受控磁碟（讓其他人保留預設值），然後按兩下列設定 ** [套用**]：
...
1. 在 [ **編輯範本** ] 刀鋒視窗上，於顯示範本內容的 區段中，以下列程式代碼取代行 **54** （`"dataDisks": [ ],`） ：
...
1. 在 az10408vmss0** 刀鋒視窗的 **[**概**觀] 區段中，按兩下 **[實例**]，選取虛擬機擴展集兩個實例旁邊的複選框，按兩下 [升級 **]，然後在系統提示您確認時，按兩下 **[**是**]。
...
1. 在 [az10408vmss0] 刀鋒視窗的 [設定] 區段中，依序按一下 [擴充功能和應用程式]、[CustomScriptExtension] 及 [解除安裝]。
...
1. 從 [Cloud Shell] 窗格中，執行下列命令以執行腳本並設定 Azure 虛擬機擴展集的磁碟：
