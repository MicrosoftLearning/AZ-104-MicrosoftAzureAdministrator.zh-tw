...
1. 在 [ **執行命令腳本** ] 刀鋒視窗中，輸入下列命令，然後按一下 [ **執行** ] 以存取 **az104-08-vm1**上裝載的網站：
...
1. 在 [ **編輯範本** ] 刀鋒視窗的區段中，顯示範本內容的 區段中，以下列程式碼取代行 **51** (`"dataDisks": [ ],`) ：
...
1. 從 [Cloud Shell] 窗格中，執行下列命令以執行腳本並設定 Azure 虛擬機器擴展集的磁片：
