...
在此實驗室中，您將了解如何實作組織的治理方案。 您了解 Azure 原則如何確保在整個組織強制執行作業決策。 您將了解如何使用資源標記來改善報告。 
...
1. 按一下代表**需要標籤及其資源值**內建原則的項目。 請花幾分鐘檢閱定義。 
...
>**注意**：按一下 [原始錯誤]**** 索引標籤，您可以找到錯誤的更多詳細資料，包括角色定義**需要具有預設值的成本中心標籤**的名稱。 部署失敗，因為您嘗試建立的儲存體帳戶沒有名為**成本中心**的標籤，且其值設定為**預設**。
...
1. 在指派清單中按一下代表**需要具有預設值的成本中心標籤**原則指派列的省略符號圖示，並使用 [刪除指派]**** 功能表項目來刪除指派。
