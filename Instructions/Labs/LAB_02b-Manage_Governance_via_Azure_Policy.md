---
lab:
  title: 實驗室 02b：透過 Azure 原則 管理治理
...
1. 在 [資源群組] 刀鋒視窗上，按一下左側功能表中的 [ **卷** 標]，然後建立新的標籤。
...
1. 按一下 [ **套用** ] 並關閉標籤版本視窗，以流覽回儲存體帳戶刀鋒視窗。 按一下儲存體帳戶上的省略號，然後選取 [ **編輯標籤** ] 以記下新標籤未自動指派給儲存體帳戶。 
...
1. 在 [**建立儲存體帳戶**] 刀鋒視窗的 [**基本] 索引**標籤上，確認您使用 [資源群組] 套用原則，並指定下列設定， (讓其他人保留預設值) ，按一下 [**檢閱**]，然後按一下 [**建立**]：
...
1. 在 [**建立儲存體帳戶**] 刀鋒視窗的 [**基本] 索引**標籤上，確認您使用 [資源群組] 套用原則，並指定下列設定， (讓其他人保持預設值) ，然後按一下 [**檢閱**]：
