...
在此工作中，您將透過 Azure 入口網站建立標籤，並將其指派給 Azure 資源群組。 標籤是治理策略的重要元件，如 Microsoft Well-Architected Framework 和雲端採用架構中所述。 標籤可讓您快速識別資源擁有者、終止日期、群組連絡人，以及貴組織認為重要的其他名稱/值組。 對於此任務，您可以指派識別資源成本中心的標籤。 
...
1. 選取 [下一步 **]**，然後移至 [標籤] **** 索引標籤。提供新標籤的資訊。
...
1. 搜尋 `Require a tag and its value on resources` 內建原則。 選取原則，然後花一分鐘時間檢閱定義。 
...
>**附註**： 按一下 原始錯誤** 索引**標籤，您可以找到有關錯誤的更多詳細資訊，包括角色定義**的名稱 需要標籤 及其資源值**。 部署失敗，因為您嘗試建立的儲存體帳戶沒有名為**成本中心**的標籤，且其值設定為**預設**。
...
1. 在指派清單中，按一下代表 **資源原則指派上** 需要標籤及其值的資料列中的省略符號圖示，然後使用 **刪除指派** 功能表項目來刪除指派。
...
1. 搜尋並選取 `Storage Account` ，然後按一下 **+ 建立**。 
...
+ Azure 標籤是包含索引鍵/值組的中繼資料。 標籤會描述您環境中的特定資源。 特別是，Azure 中的標記可讓您以邏輯方式標記資源。
