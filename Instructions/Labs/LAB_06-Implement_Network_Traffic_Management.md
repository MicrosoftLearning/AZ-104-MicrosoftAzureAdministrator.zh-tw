...
1. 使用下列設定新增對等互連 (將其他設定保持預設值)，然後按一下 [新增]：
    ...
    | 允許 'az104-06-vnet01' 存取對等互連的虛擬網路 | **確定已核取方塊， (預設) ** |
    | 允許 'az104-06-vnet01' 中的閘道將流量轉送至對等互連虛擬網路 | **確定已核取方塊** |
    | 遠端虛擬網路：對等互連連結名稱 | **az104-06-vnet2_to_az104-06-vnet01** |
    | 虛擬網路部署模型 | **資源管理員** |
    | 我知道我的資源識別碼 | 已啟用 |
    | 資源識別碼 | 您稍早在此工作中記錄的 **az104-06-vnet2** resourceID 參數值。 |
    | 允許 az104-06-vnet2 存取 az104-06-vnet01 | **確定已核取方塊， (預設) ** |
    | 允許 az104-06-vnet2 接收來自 az104-06-vnet01 的轉寄流量 | **確定已核取方塊** |
...
1. 使用下列設定新增對等互連 (將其他設定保持預設值)，然後按一下 [新增]：
    ...
    | 虛擬網路部署模型 | **資源管理員** |
    | 我知道我的資源識別碼 | 已啟用 |
    | 資源識別碼 | 您稍早在此工作中記錄 **的 az104-06-vnet3** resourceID 參數值。 |
    | 允許 az104-06-vnet3 存取 az104-06-vnet01 | **確定已核取方塊， (預設) ** |
