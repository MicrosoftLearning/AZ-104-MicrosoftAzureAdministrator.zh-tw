...
在這項工作中，您將使用範本來部署一個虛擬網路、一個網路安全組和三部虛擬機。
...
1. 在 [ **新增公用IP 位址** ] 彈出視窗上，先使用下列設定，再按兩下 [ **儲存** ] 兩次。 完成時，按一下 [Next: Backend pools] \(下一步：後端集區\)。
...
1. 在 [後端集區] 索引標籤上，搭配下列設定 (將其他設定保留為預設值) 按一下 [新增後端集區]。 按兩下 [ **新增** ]，然後按兩下 [ **儲存**]。 按 **[下一步：輸入規則**]。
