...
在此實驗室中，您將瞭解如何設定及測試公用Load Balancer和 應用程式閘道。
...
您的組織有公用網站。 您必須在不同虛擬機之間平衡傳入的公用要求。 您也需要提供來自不同虛擬機的影像和影片。 您計劃實作 Azure Load Balancer 和 Azure 應用程式閘道。 所有資源都位於相同的區域中。
...
在這項工作中，您將使用範本來部署一個虛擬網路、一個網路安全組和兩部虛擬機。
...
1. 在自定義部署頁面上，選取 **編輯器**中的 [建置您自己的範本]。

1. 在編輯範本頁面上，選取 **[載入檔案**]。

1. 找出並選取 **\\Allfiles\\Lab06\\az104-06-vms-template.json** 檔案，然後選取 [ **開啟**]。
...
1. 選取 [檢閱 + 建立]，然後選取 [建立]。
    ...
    >**注意**：等待部署完成，再移至下一個工作。 部署大約需要 5 分鐘的時間。

    >**注意**：檢閱正在部署的資源。 會有一個具有三個子網的虛擬網路。 每個子網都會有一部虛擬機。

## 工作 2：設定 Azure Load Balancer

在這項工作中，您會在虛擬網路中的兩部 Azure 虛擬機前面實作 Azure Load Balancer。 Azure 中的負載平衡器可跨資源提供第 4 層連線能力，例如虛擬機。 Load Balancer 組態包含前端 IP 位址以接受連線、後端集區，以及定義連線應如何周遊負載平衡器的規則。
...
1. 使用下列設定建立負載平衡器（讓其他人保留預設值），然後按兩下 **列 [下一步：前端IP組態**]：
    ...
    | 層 | **Regional** |
     ...
     ![建立負載平衡器頁面的螢幕快照。](../media/az104-lab06-create-lb1.png)
...
1. 在 [新增公用 IP 位址] 彈出視窗上，使用以下設定，然後依序按一下 [確定] 和 [新增]。 完成時，按一下 [Next: Backend pools] \(下一步：後端集區\)。
    ...
    >**注意：** 標準 SKU 提供靜態 IP 位址。 靜態IP位址會使用資源指派，並在刪除資源時釋放。  
...
1. 當您有時間時，請檢閱其他索引標籤，然後按兩下 [ **檢閱 + 建立**]。 確定沒有任何驗證錯誤，然後按一下 [建立]。
...
1. 從 [Load Balancer] 頁面選取 **[前端 IP 組態** ]。 複製公用IP位址。
...
1. 在 [ **虛擬網络]** 刀鋒視窗的虛擬網络清單中，按兩下 **az104-06-vnet1**。

1. 在 az104-06-vnet1** 虛擬網络刀鋒視窗的 **[設定**] 區段中，按兩下 **[子網**]，然後按兩下 [**+ 子網**]。**
...
1. 按一下 [儲存]
    ...
    > **注意**：此子網將由 Azure 應用程式閘道 使用。 應用程式閘道需要大小為 /27 或更大的專用子網路。
...
1. 在 [基本] 索引標籤上，指定下列設定 (將其他設定保留為預設值)：
    ...
    ![建立應用程式閘道頁面的螢幕快照。](../media/az104-lab06-create-appgw.png)
...
1. 按一下 [Next: Frontends >] \(下一步: 前端 >\)，然後指定下列設定 (將其他設定保留為預設值)。 完成時，按一下 [確定]。
    ...
    >**注意：** 應用程式閘道 可以同時具有公用和私人IP位址。
 
1. 按兩下 **[下一步：後端>** ]，然後 **新增後端集區**。 指定下列設定 (將其他設定保留為預設值)。 完成時，按一下 [新增]。
...
1. 按兩下 [ **新增後端集區**]。 這是映像**的後端集區**。 指定下列設定 (將其他設定保留為預設值)。 完成時，按一下 [新增]。
...
1. 按兩下 [ **新增後端集區**]。 這是影片**的後端集區**。 指定下列設定 (將其他設定保留為預設值)。 完成時，按一下 [新增]。
...
1. 移至 [ **後端目標] 索引** 標籤。完成基本信息之後，選取 **[新增** ]。
   ...
   >**注意：** 請花一分鐘的時間閱讀 Cookie 型親和性和** **連線 清空**的相關信息**。

1. 在 [ **路徑型路由** ] 區段中，選取 **[新增多個目標] 以建立路徑型規則**。 您將建立兩個規則。 按兩下 **第一個規則之後的 [新增** ]，然後在 **第二個規則之後按兩下 [新增** ]。 
...
1. 請務必儲存**** 並檢查您的變更，然後選取 **[下一步：標籤>**。 不需要變更。

1. 選取 **[下一步：檢閱 + 建立>** 然後按兩下 [ **建立**]。

    > **注意**：等候應用程式閘道執行個體建立完成。 這大約需要 5-10 分鐘的時間。 當您等待時，請考慮檢閱此頁面結尾的一些自我步調訓練連結。

1. 部署應用程式閘道之後，搜尋並選取 **az104-appgw**。

1. 在 [應用程式閘道** 資源] **的 **[監視]** 區段中，選取 **[後端健康情況**]。

1. 請確定後端集區中的兩部伺服器都顯示狀況 **良好**。

1. 在 [概**觀]** 刀鋒視窗上，複製前端公用IP位址**的值**。

1. 啟動另一個瀏覽器視窗並測試此網址 - `http://<frontend ip address>/image/`。

1. 確認您已導向至映像伺服器 （vm1）。

1. 啟動另一個瀏覽器視窗並測試此網址 - `http://<frontend ip address>/video/`。
