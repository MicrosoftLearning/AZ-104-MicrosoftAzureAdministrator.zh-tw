...
本實驗室需有 Azure 訂閱。 您的訂閱類型可能會影響此實驗室中的功能可用性。 您可以變更區域，但這些步驟是以**美國東部**為使用區域撰寫而成。
...
1. 在 [後端集區] 索引標籤上，搭配下列設定 (將其他設定保留為預設值) 按一下 [新增後端集區]。 按一下 [+ 新增]**** (兩次)，然後按一下 [下一步: 輸入規則]****。
...
1. 在 [基本] 索引標籤上，指定下列設定 (將其他設定保留為預設值)：
    ...
    ![螢幕擷取畫面: 建立應用程式網路閘道頁面。](../media/az104-lab06-create-appgw.png)
...
1. 請務必 [儲存]**** 並檢查您的變更，然後選取 [下一步:**標籤 >]**。 不需要任何變更。
