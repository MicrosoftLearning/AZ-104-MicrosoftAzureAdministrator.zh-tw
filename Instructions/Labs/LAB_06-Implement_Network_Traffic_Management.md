...
1. 在 [Frontend IP configuration] \(前端 IP 設定\) 索引標籤上，按一下 [Add a frontend IP configuration] \(新增前端 IP 設定\)，然後先使用下列設定，再依序按一下 [確定] 與 [新增]。 完成時，按一下 [Next: Backend pools] \(下一步：後端集區\)。 
