...
**注意：** **[互動式實驗室模擬](https://mslabs.cloudguides.com/guides/AZ-104%20Exam%20Guide%20-%20Microsoft%20Azure%20Administrator%20Exercise%2010)** \(英文\) 可供您以自己的步調完成此實驗室。 您可能會發現互動式模擬與託管實驗室之間稍有差異，但所示範的核心概念與想法都是相同的。 
...
1. 在 [網路監看員 - 連線疑難排解] 刀鋒視窗上，使用下列設定起始檢查 (將其他設定保留預設值)：
    ...
    > **注意**：資源群組可能需要幾分鐘的時間才會列出。 如果您不想等候，請嘗試下列動作：刪除網路監看員、建立新的網路監看員，然後重試 [連線疑難排解]。 
...
在此工作中，您將在中樞虛擬網路中的兩部 Azure 虛擬機器前端實作 Azure Load Balancer。
...
1. 在 [前端 IP 設定] 索引標籤上，按一下 [新增前端 IP 設定]，然後先使用下列設定，再依序按一下 [確定] 和 [新增]。 完成後，按一下 [下一步: 後端集區]。 
...
1. 在 [後端集區] 索引標籤上，搭配下列設定 (將其他設定保持預設值) 按一下 [新增後端集區]。 按一下 [+ 新增] \(兩次\)，然後按一下 [下一步: 輸入規則]。 
...
1. 在 [輸入規則] 索引標籤上，按一下 [新增負載平衡規則]。 使用下列設定新增負載平衡規則 (將其他設定保持預設值)。 完成時，按一下 [新增]。

    | 設定 | 值 |
    | --- | --- |
    | 名稱 | **az104-06-lb4-lbrule1** |
    | IP 版本 | **IPv4** |
    | 前端 IP 位址 | **az104-06-pip4** |
...
1. 從 Load Balancer 資源頁面中選取 [前端 IP 設定]。 複製 IP 位址。

1. 開啟另一個瀏覽器索引標籤，然後瀏覽至該 IP 位址。 確認瀏覽器視窗會顯示 **Hello World from az104-06-vm0** 或 **Hello World from az104-06-vm1** 訊息。

1. 重新整理視窗，以確認訊息會變更為另一部虛擬機器。 這可示範出負載平衡器在不同的虛擬機器之間輪轉。
...
1. 按一下 [下一步: 前端 >]，然後指定下列設定 (將其他設定保留為預設值)。 完成時，按一下 [確定]。 
...
1. 按一下 [下一步: 設定 >]，然後按一下 [+ 新增路由規則]。 指定下列設定：
...
1. 切換至 [後端目標] 索引標籤，並指定下列設定 (將其他設定保留預設值)。 完成時，按一下 [新增] (兩次)。  
...
1. 在 [az104-06-appgw5] 應用程式閘道刀鋒視窗上，複製 [前端公用 IP 位址] 的值。
