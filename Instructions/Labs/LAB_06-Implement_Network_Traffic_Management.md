...
1. 選取 [新增負載平衡規則]****。 使用下列設定新增負載平衡規則 (將其他設定保留為預設值)。  當您設定規則時，請使用參考圖示來瞭解每個設定。 完成時，按兩下 [ **儲存**]。
...
1. 移至 [ **後端目標] 索引** 標籤。完成基本信息之後，選取 **[新增** ]。
   ...
   >**注意：** 請花一分鐘的時間閱讀 Cookie 型同構**型和 **連線 清空**的相關信息**。
...
+ 應用程式閘道 標準層提供所有 L7 功能，包括負載平衡，WAF 層會新增防火牆來檢查惡意流量。
