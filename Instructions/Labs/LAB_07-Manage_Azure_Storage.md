...
1. 在 [網路 **] **索引標籤的 [公用網路存取 **] **區段中，選取 [**停用**]。 這將限制入站訪問，同時允許出站訪問。 

1. 檢閱 [資料保護]**** 索引標籤。注意預設虛刪除保留原則是 7 天。 請注意，您可以啟用 Blob 的版本設定。 接受預設值。

1. 檢閱 [加密]**** 索引標籤。注意其他安全性選項。 接受預設值。

1. 選取 [**檢閱 + 建立]**，等候驗證程式完成，然後按一下 **[建立]。**

1. 部署儲存體帳戶之後，請選取 [前往資源]****。

1. 檢閱 [概觀]**** 刀鋒視窗，以及其他可變更的設定。 這些是儲存體帳戶的全域設定。 注意儲存體帳戶可用於 Blob 容器、檔案共用、佇列和資料表。

1. 在 [**安全性 + 網路]** 刀鋒視窗中，選取 [**網路]。** 請注意 **：公用網路存取** 已停用。

    + 選取 [**管理]**，並將 [公用網路存取 **] **設定變更為 **[已啟用**]。 
    + 將公用網路存取範圍**變更**為**從選取的網路**啟用。
    + 在 [IPv4 位址] **區段中，選取 [**新增您的用戶端 IPv4 位址**]。**
    + 儲存您的變更。
  
1. 在 [資料管理 **] **刀鋒視窗中，選取 [**備援]。** 注意主要和次要資料中心位置的相關資訊。

1. 在 [資料管理] **刀鋒視窗中，選取 [**生命週期管理**]，然後選取 [**新增規則**]。**

    + 將規則**命名**為 `Movetocool`。 注意限制規則範圍的選項。 按一下 [下一步] 。 
    
    + 在 [新增規則] **頁面上，*如果基底 Blob 上次修改`30`的時間超過幾天，*則** **[移至非公開儲存體]。**** 請注意您的其他選擇。 
...
1. 在 [資料儲存體 **] **刀鋒視窗中，選取 **[容器]。** 

1. 按一下 **+ 新增容器** ，然後 **使用** 下列設定建立容器：
...
1. 選取您的上傳檔案，然後檢閱省略符號 （...） 選項，包括 **[下載**]、[刪除]**、[****變更層級**] 和 **[取得租用**]。

1. 複製檔案 **URL** （[設定] --[> 屬性] 刀鋒視窗） ，然後貼到新的 **Inprivate** 瀏覽視窗中。
...
1. 瀏覽回您上傳的檔案，然後選取最右邊的省略符號 （...），然後選取 **[產生 SAS]** 並指定下列設定 （讓其他設定保留預設值）：
...
1. 在 Azure 入口網站中，流覽回您的儲存體帳戶，在 [資料儲存體 **] **刀鋒視窗中，按一下 [**檔案共用]。**
...
1. 在入口網站中，搜尋並選取 `Virtual networks`。
...
1. 在 [**安全性 + 網路]** 刀鋒視窗中，選取 [**網路]。**

1. 在公用網路存取**下，** 選取 **[管理**]。 

1. 選取 [新增虛擬網路 **]，然後**選取 [** 新增現有的網路**]。

1. 選取 **[vnet1** ] 和 **預設** 子網，然後選取 **[新增**]。

1. 在 IPv4 位址**區**段中，**刪除**您的電腦 IP 位址。 應該只允許來自虛擬網路的流量。 
...
1. 選取 [儲存體瀏覽器]****，然後**重新整理**頁面。 瀏覽至您的檔案共用或 Blob 內容。  
    ...
    >**注意：** 您應該會收到「沒有執行這項作業的權限」** 訊息。 您不是從虛擬網路連線。 這可能需要幾分鐘的時間才會生效。 您仍然可以檢視檔案共用，但無法檢視儲存體帳戶中的檔案或 Blob。 
