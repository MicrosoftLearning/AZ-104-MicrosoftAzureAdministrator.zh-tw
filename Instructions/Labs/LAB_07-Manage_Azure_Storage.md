...
1. 按兩下 [驗證方法]** 標籤的 **[**切換至 Microsoft Entra 使用者帳戶**] 連結。

    > **注意**：當您變更驗證方法時，您會看到錯誤（錯誤是 *「您沒有許可權使用用戶帳戶搭配 Microsoft Entra 列出數據」）。* 這是預期的結果。  
...
1. 按兩下 [**檢閱 + 指派]，然後**檢閱 + 指派****，然後返回 **** az104-07-container 容器**的 **[概觀] 刀鋒視窗，並確認您可以將 [驗證方法] 變更為 [切換至 Microsoft Entra 使用者帳戶]。
...
1. 按兩下 **[+ 檔案分享**]，然後在 [基本] 索引**標籤上**提供檔案共享名稱 **az104-07-share**。 檢閱此索引標籤上的其他設定。 

1. 移至 [**備份]** 索引標籤，並確定**未**核取 [啟用備份****]。

1. 按兩下 [ **檢閱並建立**]，然後按兩下 **[建立**]。 等候檔案共用部署。 

1. 按兩下新建立的檔案共用，並記下 az104-07-share** 刀鋒視窗上**可用的資訊。

1. 按兩下 **[瀏覽** ]，並注意新檔案共享中沒有任何檔案或資料夾。 按一下 **連線**。
...
1. 流覽回 az104-07-share \| 瀏覽**檔案共用刀鋒視窗，按兩下 **[重新**整理]，並確認 **az104-07-folder** 出現在資料夾**清單中。
