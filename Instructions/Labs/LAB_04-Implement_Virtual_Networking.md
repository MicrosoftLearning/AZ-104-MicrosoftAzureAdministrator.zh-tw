...
1. 從 [Cloud Shell] 窗格中，執行下列命令來測試新建立 DNS 區域中**az104-04-vm0** DNS 記錄集的外部名稱解析， (將預留位置取代為您稍早在此工作中記下的名稱**伺服器 1**，並將 `[domain name]` 占位 `[Name server 1]` 符取代為您稍早在此工作中建立的 DNS 網功能變數名稱稱) ：
...
1. 從 [Cloud Shell] 窗格中，執行下列命令來測試新建立 DNS 區域中**az104-04-vm1** DNS 記錄集的外部名稱解析， (將預留位置取代為您稍早在此工作中記下的名稱**伺服器 1**，並將 `[domain name]` 占位 `[Name server 1]` 符取代為您稍早在這項工作中建立的 DNS 功能變數名稱) ：
