...
1. 回頭仔細閱讀檔案，並確定一切看起來都正確。 使用資源名稱和IP位址的架構圖表。 
...
1. 選取 **具有製造變更template.json** 檔案，然後選取 [ **儲存**]。

1. 選取 [編輯參數 **]，然後**選取 **[載入檔案**]。

1. 選取 **具有製造變更parameters.json** 檔案，然後選取 [ **儲存**]。

1. 確定已選取您的資源群組 **az104-rg4** 。 
...
>**注意：** 此時，您會將 ASG 與虛擬機產生關聯。 這些計算機將受到您在下一個工作中建立的輸入NSG規則所影響。  

### 建立網路安全組並將它與 CoreServicesVnet 產生關聯
...
1. 展開 [ **DNS 管理]** 刀鋒窗口，然後選取 **[+ 記錄集**]。 **針對需要私人名稱解析支援的每個虛擬網路新增** 虛擬網路連結記錄。
...
1. 選取 [**確定**]，並確認您的網域有名為 www** 的 **A 記錄集。

1. 開啟命令提示字元，然後執行下列命令。 如果您已變更功能變數名稱，請進行調整。 
...
1. 展開 [ **DNS 管理]** 刀鋒視窗，然後選取 **[虛擬網络連結**]。 設定連結。 
...
1. 從 [ **DNS 管理]** 刀鋒視窗中，選取 **[+ 記錄集**]。 您現在可為每部需要私人名稱解析支援的虛擬機器新增記錄。
