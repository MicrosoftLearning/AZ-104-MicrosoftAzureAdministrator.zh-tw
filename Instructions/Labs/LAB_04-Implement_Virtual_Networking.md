...
1. 移至 [IP 位址]**** 索引標籤。
    ...
    |  **選項**         | **值**            |
    | ------------------ | -------------------- |
    | IPv4 位址空間 | 將預先填入的 IPv4 位址空間取代為 `10.20.0.0/16` （分隔項目）  |

1. 選取 [+ 新增子網路]****。 完成每個子網路的名稱和位址資訊。 請務必為每個新子網路選取 [新增]****。 

    >**注意：** 請務必刪除預設子網路 - 在建立其他子網路之前或之後。
...
1. 回頭仔細閱讀檔案，並確定一切看起來都正確。 使用架構圖來取得資源名稱和 IP 位址。 
...
1. 在入口網站中，搜尋並選取 `Deploy a custom template`。

1. 選取 [在編輯器中建置您自己的範本]****，然後選取 [載入檔案]****。

1. 選取**包含製造變更的template.json**檔案，然後選取儲存****。

1. 選取 [編輯參數 **]，然後**選取 **[載入檔案**]。

1. 選取 **包含製造變更的parameters.json** 檔案，然後選取 [ **儲存**]。

1. 請確定已選取您的資源群組 **az104-rg4** 。 
...
1. 請注意 **AllowInternetOutBound** 規則。 另請注意，無法刪除規則，且優先順序為 65001。
...
1. 展開 [DNS 管理 **] 刀鋒視窗，然後選取 **[記錄集**]。** 按一下 [+新增]****。 
...
1. 選取 [新增 **]，** 並確認您的網域具有名為 www** 的 **A 記錄集。

1. 開啟命令提示字元，然後執行下列命令。 如果您更改了域名，請進行調整。 
...
1. 展開 [DNS 管理] **** 刀鋒視窗，然後選取 [**虛擬網路連結]。** 設定連結。 
...
1. 從 [ **DNS 管理]** 刀鋒視窗中，選取 **[+ 資料集**]。 您現在可為每部需要私人名稱解析支援的虛擬機器新增記錄。
