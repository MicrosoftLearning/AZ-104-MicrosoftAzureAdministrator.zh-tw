...
1. 從虛擬機器頁面中，選取 [ **建立** ]，然後選取 [ **虛擬機器**]。

1. 在 [基本] 索引標籤上，使用下列資訊來完成表單，然後選取 [ **下一步：磁碟>**]。 如果未指定設定，則請保留預設值。
    ...
    | 可用性選項 | 不需要基礎結構備援 |
    | 安全性類型 | **標準** |
    | 圖片 （查看所有圖片） | **Windows Server 2019 資料中心 - x64 Gen2** （請注意您的其他選擇） |
    | 大小 | **Standard_DS2_v3** |
    | 使用者名稱 | `localadmin` | 
    | 密碼 | **提供複雜密碼** |
    | 公用輸入連接埠 | **None** |
...
1. 在 [磁碟]** 索引標籤上，採用預設值，然後選取 [**下一步：網路>**]。**
...
1. 在 CoreServicesVnet 的 [對等互連] 底下，選取 **[+ 新增**]。 如果未指定，則採用預設值。 

    | **參數**                                    | **ReplTest1**                             |
    | --------------------------------------------- | ------------------------------------- |
    | 對等互連連結名稱                             | `ManufacturingVnet-to-CoreServicesVnet` |
    | 虛擬網路    | **ManufacturingVnet （az104-rg5）**  |
    | 允許 'CoreServicesVnet' 存取 'ManufacturingVnet'            | 已選取 (預設) |
    | 允許 'CoreServicesVnet' 接收來自 'ManufacturingVnet' 的轉送流量 | 已選取 |
...
5. 在 CoreServicesVnet 的 [對等互連] 底下，確認 **已列出 CoreServicesVnet 對 ManufacturingVnet** 對等互連。 重新整理頁面以確保 [對等互連狀態]**** 為 [已連線]****。
...
1. 選取 [RunPowerShellScript]**** 並執行 **Test-NetConnection** 命令。 請務必確認 **CoreServicesVM** 的私人 IP 位址。
    ...
    ```
1. 指令碼可能需要幾分鐘的時間才能逾時。頁面頂端會顯示資訊訊息 *Script 執行正在進行中...*
...
在此工作中，您想要控制周邊子網路和內部核心服務子網路之間的網路流量。 虛擬網路設備將會安裝在周邊子網路中，而且所有流量都應該路由傳送到該處。 

1. 搜尋並選取 `CoreServicesVnet`。

1. 選取 [子網路]**，然後**選取 **[+ 子網路**]。 請務必選取 [ **新增** ] 以儲存您的變更。 
...
1. 在 Azure 入口網站 中，搜尋並選取 `Route tables`，選取 **[+ 建立]。**

1. 輸入下列詳細資料，選取**檢閱 + 建立**，然後選取**建立。** 
...
1. 選取資源 （而非核取方塊） **rt-CoreServices**

1. 展開 設定 然後**選取 路由** ，然後**選取 **+ 新增**。** 建立從未來網路虛擬設備 （NVA） 到 CoreServices 虛擬網路的路由。 
...
1. 選取 [子網路]**，然後**選取 **[+ 關聯]。** 完成組態。

    | 設定 | 值 | 
    | --- | --- |
    | 虛擬網路 | **CoreServicesVnet （az104-rg5）** |
    | 子網路 | **核心** |    
