...
1. 選取 **虛擬機旁的 [啟用** ]，然後在 **[Azure 監視器 - 深入解析上線** ] 刀鋒視窗上啟用。
...
1. 選取訂用帳戶的方塊，然後選取 [ **套用**]。 此警示會套用至訂用帳戶中的任何虛擬機。 或者，您可以只指定一部特定的機器。 
...
1. 在 Azure 入口網站 中，搜尋並選取 `Monitor`，然後按兩下 [**記錄**]。

1. 如有必要，請關閉啟動顯示畫面。 

1. 如有必要，請選取您的訂**用帳戶範圍**。 選取**套用**。 

1. 在 [查詢]**** 索引標籤中，選取 [虛擬機器]**** (左側窗格)。 您可能需要重新開啟刀鋒視窗。

    ![[查詢] 索引標籤的螢幕快照。](../media/az104-lab11-queries.png)

1. 檢閱可用的查詢。 **執行** (將滑鼠停留在查詢上) **計數活動訊號**查詢。

1. 您應該會在虛擬機器執行時收到活動訊號計數。

1. 在畫面右側選取 [簡單模式] 旁**的下拉式清單，選擇 **[KQL 模式**]。** 檢閱查詢。 此查詢會使用 [活動訊號]** 資料表。
...
1. 使用此查詢取代查詢，然後按一下 [執行]****。 檢視產生的圖表。 
   ...
   ```
    ...
    >**注意：** 如果查詢未正確貼上，請嘗試貼到 [記事本]，然後將它複製並重新貼到查詢字段中。
