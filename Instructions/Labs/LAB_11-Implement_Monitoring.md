...
1. 將 Allfiles Labs\\11\\az104-11-vm-template.json** 實驗室文件下載**\\到您的\\計算機。
...
1. 找到並選取**\\Allfiles\\Labs\\11\\az104-11-vm-template.json** 檔案，然後選取 [**開啟**]。
...
1. 選取虛擬機器旁的 [啟用]**，然後**在**[Azure 監視器 - 深入解析上線**] 刀鋒視窗上選取 [啟用]。
...
1. 選取訂用帳戶的方塊，然後選取 [**** 套用]。 此警示將套用至訂用帳戶中的任何虛擬機器。 或者，您可以只指定一部特定的機器。 
...
1. 繼續處理警示。 選取 [使用動作群組]，**然後在 [選取動作群組 **]** 刀鋒視窗中選取 [建立動作群組 **]。****
...
1. 建立動作群組後，請移至 [**下一步：詳細資料>** 索引標籤，然後為每個設定輸入下列值。
...
1. 核取 [套用強制刪除]**** 的方塊。 核取底部的方塊，確認您要刪除資源，然後選取 **[刪除]。** 

1. 在標題列中選取**通知**圖示，並等候系統成功刪除**vm0**。

1. 您應該會收到具有下列內容的通知電子郵件:「**重要通知:Azure 監視器警示 VM 已刪除已啟動...** 如果沒有收到，請開啟您的電子郵件程式，然後從尋找寄件者為azure-noreply@microsoft.com 的電子郵件。
...
1. 在 Azure 入口網站 中，搜尋並選取`Monitor`，然後按一下 **[記錄]。**

1. 如有必要，請關閉啟動顯示畫面。 

1. 如有必要，請選取範圍，即您的**訂閱**。 選取**套用**。 

1. 在 [查詢]**** 索引標籤中，選取 [虛擬機器]**** (左側窗格)。 您可能需要重新打開刀片。

    ![查詢索引標籤的螢幕擷取畫面。](../media/az104-lab11-queries.png)

1. 檢閱可用的查詢。 **執行** (將滑鼠停留在查詢上)**計數活動訊號**查詢。

1. 您應該會在虛擬機器執行時收到活動訊號計數。

1. 在畫面右側，選取 [簡單模式] 旁**的下拉式清單，選擇 **[KQL 模式**]。** 檢閱查詢。 此查詢會使用 [活動訊號]** 資料表。
...
1. 使用此查詢取代查詢，然後按一下 [執行]****。 檢視產生的圖表。 
   ...
   ```
    ...
    >**附註：** 如果查詢未正確貼上，請嘗試貼到記事本中，然後複製並重新貼到查詢欄位中。
