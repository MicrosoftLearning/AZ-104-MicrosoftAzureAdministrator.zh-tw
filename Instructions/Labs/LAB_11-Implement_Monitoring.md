...
1. 下載 **\\Allfiles\\Labs\\11\\az104-11-vm-template.json** 實驗室檔案到您的電腦。
...
1. 找到並選擇 **\\Allfiles\\Labs\\11\\az104-11-vm-template.json** 檔案，選擇 **開啟**。
...
1. 選擇訂閱選項，然後選擇 **「** 應用」。 此警示將適用於訂閱中的任何虛擬機。 或者，您可以只指定一部特定的機器。 
...
1. 繼續處理警示。 移到**動作**標籤，選擇**使用動作群組**，然後在選擇動作群組**刀片中**選擇**建立動作群組**。
...
1. 建立動作群組後，移至 **「下一步：細節>** 」標籤，並為每個設定輸入以下數值。
...
1. 核取 [套用強制刪除]**** 的方塊。 勾選底部確認你希望刪除資源的方框，並選擇 **刪除**。 
...
1. 在 Azure 入口網站中，搜尋並選擇 `Monitor`，然後點選**日誌**。

1. 必要時關閉啟動畫面。 

1. 如有需要，請選擇一個範圍，也就是您的 **訂閱**。 選取**套用**。 

1. 在 [查詢]**** 索引標籤中，選取 [虛擬機器]**** (左側窗格)。 你可能需要重新打開刀片。

    ![查詢分頁的截圖。](../media/az104-lab11-queries.png)

1. 檢閱可用的查詢。 **執行** (將滑鼠停留在查詢上) **計數活動訊號**查詢。

1. 您應該會在虛擬機器執行時收到活動訊號計數。

1. 在螢幕右側，選擇簡單模式**旁**的下拉選單，選擇 **KQL 模式**。 檢閱查詢。 此查詢會使用 [活動訊號]** 資料表。
...
1. 使用此查詢取代查詢，然後按一下 [執行]****。 檢視產生的圖表。 
   ...
   ```
    ...
    >**注意：** 如果查詢沒有正確貼上，請嘗試先貼到記事本，再複製並重新貼上到查詢欄位。
